...
class TestConfiguration < BridgetownUnitTest
  ...
  context "#read_config_file" do
    ...
    should "not raise an error on empty files" do
      allow(YAML).to receive(:safe_load_file).with(File.expand_path("empty.yml")).and_return(false)
  ...
  context "#read_config_files" do
    ...
    should "continue to read config files if one is empty" do
      allow(YAML).to receive(:safe_load_file).with(File.expand_path("empty.yml")).and_return(false)
      allow(YAML).to receive(:safe_load_file).with(File.expand_path("not_empty.yml")).and_return(
        "foo" => "bar"
      )
      Bridgetown.logger.log_level = :warn
  ...
  context "loading configuration" do
    ...
    should "fire warning with no bridgetown.config.yml" do
      allow(YAML).to receive(:safe_load_file).with(@path) do
    ...
    should "load configuration as hash" do
      allow(YAML).to receive(:safe_load_file).with(@path).and_return({})
    ...
    should "fire warning when user-specified config file isn't there" do
      allow(YAML).to receive(:safe_load_file).with(@user_config) do
  ...
  context "loading config from external file" do
    ...
    should "load default plus posts config if no config_file is set" do
      allow(YAML).to receive(:safe_load_file).with(@paths[:default]).and_return({})
    ...
    should "load multiple config files" do
      ...
      allow(YAML).to receive(:safe_load_file).with(@paths[:default]).and_return({})
      allow(YAML).to receive(:safe_load_file).with(@paths[:other]).and_return({})
