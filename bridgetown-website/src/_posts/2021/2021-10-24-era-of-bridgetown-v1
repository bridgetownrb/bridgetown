...
Named after the famed Pearl District on Portland’s west side, I’m pleased to announce the first public alpha release of **Bridgetown 1.0** (technically 1.0.0.alpha5). v1 is _chock full_ of major advancements for the platform. [The “edge” documentation is available here](https://edge.bridgetownrb.com)—or skip further below for upgrade notes from v0.x.

But before we get to all that, a **second announcement**!

As you may know, Bridgetown is entirely funded by “viewers like you,” and we’ve taken on (and partially executed!) [an ambitious roadmap](https://www.bridgetownrb.com/future/roadmap-to-1.0/) heading into 2022. In other words, **we need your help** to get v1 finalized, polished, and ready for production—as well as further all the work which goes into writing documentation, creating tutorials, attending to design & branding, and generally shepherding the ecosystem.

Therefore, **[we’ve launched a dedicated fundraising site for Bridgetown](https://fundraising.bridgetownrb.com).** We hope you join the campaign to get v1 over the finish line, and please help us spread the word so other interested Rubyists and web developers may contribute as well.
...
Bridgetown 1.0 takes advantage of this evolving paradigm by providing a streamlined path for booting a site up in-memory. This means you can write a server-side API to render content whenever it is requested. While this API could be built with any Rack-based framework, you might very well want to take advantage of our native Roda integration because **it’s just that easy to add a dynamic API route**.
...
SSR is great for generating preview content on-the-fly, but you can use it for any number of instances where it’s not feasible to pre-build your content. In addition, you can use SSR to “refresh” stale content…for example, you could pre-build all your product pages statically, but then request a newer version of the page (or better yet, just a tiny snippet of it) whenever the static page is viewed which would then contain the up-to-date pricing (perhaps coming from a PostgreSQL database or some other external data source). And if you cache _that_ data using Redis in, say, 10-minute increments, you’ve just built yourself an extremely performant e-commerce solution. This is only one example!
...
Testing is quite straightforward as well. Simply place `.test.rb` files alongside your routes, and you’ll be able to use Capybara to write **fast** integration tests including interactions requiring Javascript (assuming Cupite is also installed).
...
We encourage you to [try Bridgetown 1.0.0.alpha5 today](https://edge.bridgetownrb.com) and jump into our Discord chat to let us know what you think. Your feedback and ideas are invaluable in shaping the future of Bridgetown and helping make it the best site generator for Rubyists everywhere.
...
You will need to add a few additional files to your project, so we suggest using `bridgetown new` to create a separate project, then copy those files over:
...
Finally, you an remove `start.js` and `sync.js` and well as any scripts in `package.json` besides `webpack-build` and `webpack-dev` (and you can also remove the `browser-sync` and `concurrently` dev dependencies in `package.json`).
...
The other major change you’ll need to work on in your project is switching your plugins/templates to use resources. There’s a fair degree of [documentation on this now on our edge site](https://edge.bridgetownrb.com/docs/resources). FYI, if you used the Document Builder API in the past, you’ll need to upgrade to the [Resource Builder API](https://edge.bridgetownrb.com/docs/plugins/external-apis).

We’ve added an “upgrade-help” channel in our Discord chat so if you get totally suck, let the community give you a leg up! (Access to the repo in question is almost always a given in order to troubleshoot, so if your code needs to remain private, please create a failing example we can access on GitHub.)

---- 

You’ve made it this far? Wow! Thanks so much for your interest in the future of Bridgetown—and if you haven’t already, _please_ [join our fundraising campaign](https://fundraising.bridgetownrb.com) so we can keep adding nifty goodies to Bridgetown and help make your websites awesome.