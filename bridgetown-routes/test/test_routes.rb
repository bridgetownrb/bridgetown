...
class TestRoutes < BridgetownUnitTest
  ...
  describe "Roda-powered Bridgetown server" do # rubocop:todo Metrics/BlockLength
    it "returns the static index page" do
      get "/"
      expect(last_response).is? :ok?
      expect(last_response.body) == "<h1>Index</h1>"
    end

    it "returns the dynamic index page if present" do
    ...
    it "returns HTML for the howdy route" do
    ...
    it "returns HTML for a route localized in english" do
      get "/localized"
      assert_equal "<h1>Localized for en - en</h1>\n", last_response.body
    end

    it "returns HTML for a route localized in italian" do
      get "/it/localized"
      expect(last_response.body) == "<h1>Localized for it - it</h1>\n"
    end

    it "returns HTML for nested index RESTful route" do
      get "/nested"
      assert_equal "<h1>Nested Index</h1>\n", last_response.body
    end

    it "returns HTML for nested item RESTful route" do
      get "/nested/123-abc"
      assert_equal "<h1>Nested Page with Slug: 123-abc</h1>\n", last_response.body
    end

    it "returns JSON for a base route (no template)" do
      get "/bare_route/4"
      assert_equal({ numbers: [2, 4, 6, 8] }.to_json, last_response.body)
    end

    it "outputs csrf hidden input tag" do
      get "/form"
      expect(last_response.body) << '<form><input type="hidden" name="_csrf" value="'
    end

    it "returns the proper route within an island" do
      get "/paradise" do
