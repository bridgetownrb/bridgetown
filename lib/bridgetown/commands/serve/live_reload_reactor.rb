    ...
    class Serve
      class LiveReloadReactor
        ...
        def stop
          ...
          Bridgetown.logger.debug "LiveReload Server:", "halted"
        end
        ...
        def reload(pages)
          pages.each do |p|
            ...
            Bridgetown.logger.debug "LiveReload:", "Reloading #{p.url}"
            Bridgetown.logger.debug "", json_message
        ...
        def connect(websocket, handshake)
          ...
          if @connections_count == 1
            ...
            Bridgetown.logger.info "LiveReload:", message
          end
          websocket.send(
            JSON.dump(
              :command    => "hello",
              :protocols  => ["http://livereload.com/protocols/official-7"],
              :serverName => "bridgetown"
            )
          )
        ...
        def log_error(error)
          Bridgetown.logger.error "LiveReload experienced an error. " \
